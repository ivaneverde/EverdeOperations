...
export function buildBotProfilePromptBlock(profile: BotProfile): string {
  ...
  if (profile === "full") {
    return [
      ...
      "WCRO: call get_wcro_dashboard. Lead with published segments / top_pools_by_market / transfers / reps. For a **specific store's net need / store needs**, pass **store=** (e.g. store=774) and lead with **by_store_net_need** (gross_need_u = store×pool net need). For **overstock / overstocked items**, pass **store=** and lead with **by_store_overstock** (official WCRO Rule 1/2 Excess $) — do NOT invent overstock from YTD. For **QC release / citrus zone moves / ops transfers** use **focus=ops**. For **account-manager setup fixes** (wrong plant, not set up, other-market) use **focus=am_setup** and **account_manager=** (Brian/Jae/John). For **xref suspect multi-plant SKUs** use **focus=xref**. Do NOT say store net need or overstock is unavailable when those *_available flags are true. When asked for SKUs/items use retailer_pool_sku + top_items + everde_item_codes. NN = Net Need. Cite snapshot date once. Do not deny pool data when top_pools_by_market is present. Label any YTD+farm cross-check as hypothesis, not the official WCRO order.",
  ...
  if (profile === "hd") {
    return [
      ...
      "- WCRO: call get_wcro_dashboard. Answer helpfully from published extract — for top pools / spread prep use **top_pools_by_market**. For a **specific store's net need**, pass **store=** (e.g. store=0614) and lead with **by_store_net_need** (gross_need_u). For **overstock**, pass **store=** and lead with **by_store_overstock** (official Excess $) — do not invent from YTD. For **QC / citrus / ops transfers** use **focus=ops**. For Brian/Jae setup fixes use **focus=am_setup**. For xref suspects use **focus=xref**. Do NOT say you lack store net need/overstock when those *_available flags are true. When asked for SKUs/items, show **retailer_pool_sku**, **top_items** (item + item_description), and **everde_item_codes** — do not stop at genus alone. Cite snapshot date once.",
  ...
  return [
    ...
    "- WCRO: call get_wcro_dashboard. Answer helpfully from published extract — for top pools / spread prep use **top_pools_by_market**. For a **specific store's net need / store needs** (e.g. Lowe's 774), pass **store=774** and lead with **by_store_net_need** (gross_need_u = store×pool Gross Need). For **overstock / overstocked items** (e.g. Upland 1041), pass **store=1041** and lead with **by_store_overstock** (Rule 1/2 Excess $ from WCRO Store Overstock tab) — do NOT invent overstock from YTD heuristics. For **QC / citrus / ops transfers** use **focus=ops**. For John's Lowe's setup fixes use **focus=am_setup account_manager=John**. For xref suspects use **focus=xref**. Do NOT say store net need or overstock is unavailable when those *_available flags are true. When asked for SKUs/items, show **retailer_pool_sku**, **top_items** (item + item_description), and **everde_item_codes** — do not stop at genus alone. Cite snapshot date once. LOW S.CA includes AZ/NV/NM/UT — not CA-only.",
    "- NN = Net Need. NN Plan (plan-driven) ≠ NN Cust Store (store-summed demand-sensed) ≠ NN Cust Pool (pool-netted). Explain briefly when asked.",
