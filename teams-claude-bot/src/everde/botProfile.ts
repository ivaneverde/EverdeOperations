...
export function buildBotProfilePromptBlock(profile: BotProfile): string {
  ...
  if (profile === "full") {
    return [
      ...
      "WCRO: call get_wcro_dashboard. Lead with published segments / top_pools_by_market / transfers / reps. For a **specific store's net need / store needs**, pass **store=** (e.g. store=774) and lead with **by_store_net_need** (gross_need_u = store×pool net need). Do NOT say store net need is unavailable when by_store_net_need_available is true. When asked for SKUs/items use retailer_pool_sku + top_items + everde_item_codes. NN = Net Need. Cite snapshot date once. Do not deny pool data when top_pools_by_market is present. Label any YTD+farm cross-check as hypothesis, not the official WCRO order.",
  ...
  if (profile === "hd") {
    return [
      ...
      "- WCRO: call get_wcro_dashboard. Answer helpfully from published extract — for top pools / spread prep use **top_pools_by_market**. For a **specific store's net need**, pass **store=** (e.g. store=0614) and lead with **by_store_net_need** (gross_need_u). Do NOT say you lack store net need when by_store_net_need_available is true. When asked for SKUs/items, show **retailer_pool_sku**, **top_items** (item + item_description), and **everde_item_codes** — do not stop at genus alone. Cite snapshot date once.",
  ...
  return [
    ...
    "- WCRO: call get_wcro_dashboard. Answer helpfully from published extract — for top pools / spread prep use **top_pools_by_market**. For a **specific store's net need / store needs** (e.g. Lowe's 774), pass **store=774** and lead with **by_store_net_need** (gross_need_u = store×pool Gross Need). Do NOT say store net need is unavailable or only at pool/market grain when by_store_net_need_available is true. When asked for SKUs/items, show **retailer_pool_sku**, **top_items** (item + item_description), and **everde_item_codes** — do not stop at genus alone. Cite snapshot date once. LOW S.CA includes AZ/NV/NM/UT — not CA-only.",
    "- NN = Net Need. NN Plan (plan-driven) ≠ NN Cust Store (store-summed demand-sensed) ≠ NN Cust Pool (pool-netted). Explain briefly when asked.",
