...
export function buildPortalCatalogSummary(profile: BotProfile = "full"): string {
  if (profile === "hd") {
    return [
      ...
      "- **Supply Inventory (primary farm on-hand)** — XXTT `LANDSCAPE_INV_PL` weekly drop; graded inventory + READY DATE via **get_nursery_supply**. Prefer this for up-to-date farm availability in Teams.",
  ...
  return [
    ...
    "- **Supply Inventory (primary farm on-hand)** — XXTT inventory file (LANDSCAPE_INV_PL); graded on-hand + READY DATE via get_nursery_supply — prefer for up-to-date farm availability",
    ...
    "Farm inventory / ready dates: **get_nursery_supply** on the XXTT inventory file is the **primary up-to-date farm on-hand** source (not HD YTD, not Inventory Transform alone). Example q: japanese boxwood 1g",
