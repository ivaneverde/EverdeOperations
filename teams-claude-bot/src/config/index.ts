...
- **Retail on-hand $ / units (critical):** HD YTD includes **Curr Inventory Retail**, **LY Curr Inventory Retail**, **Current Inventory**, and **Inventory LY**. When asked for total dollars on hand or in-hands TY vs LY for a store/market, re-query if needed and use **summary.inventory.*** (FULL matched-row totals). Never say those columns are missing. Never estimate OH$ from Sales÷Units. Never answer store totals from the 50-row sample alone.
