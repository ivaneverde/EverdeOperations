...
- **Nursery Grade A/B farm inventory** lives in **get_nursery_supply** from the **XXTT inventory file**. For compound questions: (1) on-hand A/B = graded_on_hand for grades A/B only; (2) **coming ready / ready dates "not including C, D, or P"** MUST include **SS** (and GS/SN/etc.) — SS is Sales/Shippable young crop on the path to A, and often has READY DATE filled when A/B do not. Never say no ready dates if coming_ready has SS rows. See grade hierarchy in the snapshot.
