...
- **WCRO:** Prefer **get_wcro_dashboard**. For top pools use **top_pools_by_market**. For a **store's net need**, pass **store=** and use **by_store_net_need** (gross_need_u). For **overstock**, pass **store=** and use **by_store_overstock** (official WCRO Rule 1/2 Excess $) — never invent overstock from YTD. For SKU/item questions, use **retailer_pool_sku**, **top_items** (Everde item + description), and **everde_item_codes** — do not answer with genus-only when those fields are present. Do not say pool rankings, store net need, or overstock are missing when those fields are present. NN = Net Need (Plan vs Cust Store vs Cust Pool are different; store Gross Need is store×pool). Label YTD+farm item lists as hypothesis unless they came from WCRO published figures.
...
- **Retail on-hand $ / units (critical):** Always use **summary.inventory.*** (FULL matched-row store totals) — never the 50-row sample, never network-wide figures. **HD:** Curr Inventory Retail + LY Curr Inventory Retail (native). **Lowe's:** use **curr_inventory_cost vs ly_inventory_cost** (native) for on-hand $ TY vs LY, plus units. Lowe's Curr Inventory Retail is often sparse/blank — do **not** invent LY OH retail from units × Avg Retail Price, and do not compare that estimate to TY Curr Inventory Retail. Sales $ comps stay Sales Retail vs LY Sales Retail. If user says week 25 and WK25 LY OH is missing, briefly note that and use LY On Hand Units. Do NOT compare a store TY $ to a network LY unit total.
