...
- **WCRO:** Prefer **get_wcro_dashboard**. For top pools use **top_pools_by_market**. For a **store's net need**, pass **store=** and use **by_store_net_need** (gross_need_u). For SKU/item questions, use **retailer_pool_sku**, **top_items** (Everde item + description), and **everde_item_codes** — do not answer with genus-only when those fields are present. Do not say pool rankings or store net need are missing when those fields are present. NN = Net Need (Plan vs Cust Store vs Cust Pool are different; store Gross Need is store×pool). Label YTD+farm item lists as hypothesis unless they came from WCRO published figures.
