...
| **Group chats** | Teams does not pass file bytes via Bot Framework — bot uses **Microsoft Graph** (`Chat.Read.All` + `Files.Read.All`). See [`GRAPH_GROUP_FILES.md`](GRAPH_GROUP_FILES.md). Reinstall app after manifest upgrade. **Follow-up questions** reuse cached file text in the same chat — users do not need to re-upload. |
