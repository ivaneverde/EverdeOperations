...
def parse_args():
    ...
    p.add_argument('--region', default='NOR CAL', choices=['NOR CAL', 'OR'],
                   help='Region label for meta + default output filename')
...
def extract_from_workbook(wb_path, region='NOR CAL'):
...
def extract_from_existing_workbook(wb_path, out_path, region='NOR CAL'):
    """
    Fast path: if the output workbook already exists (pre-generated by
    build_norcal_workbook_patched.py / build_or_workbook_patched.py), just extract JSON.
    """
    print(f"Extracting from: {wb_path}")
    data = extract_from_workbook(wb_path, region=region)
...
def _region_defaults(region):
    if region == 'OR':
        return {
            'build': 'build_or_workbook_patched.py',
            'glob': 'OR_Forward_Looking*.xlsx',
            'out_name': 'or_sales_plan_data.json',
            'model': 'or_forward_patched.py',
        }
    return {
        'build': 'build_norcal_workbook_patched.py',
        'glob': 'NOR_CAL_Forward_Looking_*.xlsx',
        'out_name': 'sales_plan_data.json',
        'model': 'nor_cal_forward_patched.py',
    }


if __name__ == '__main__':
    # Fast extract: workbook path only (optional --region OR)
    if len(sys.argv) >= 2 and sys.argv[1].endswith('.xlsx') and '--inv' not in sys.argv:
        region = 'OR' if '--region' in sys.argv and 'OR' in sys.argv else 'NOR CAL'
        if 'OR' in Path(sys.argv[1]).name.upper():
            region = 'OR'
        defaults = _region_defaults(region)
        wb = sys.argv[1]
        out = str(Path(wb).with_name(defaults['out_name']))
        if '--out' in sys.argv:
            i = sys.argv.index('--out')
            if i + 1 < len(sys.argv):
                out = sys.argv[i + 1]
        extract_from_existing_workbook(wb, out, region=region)
        sys.exit(0)
    ...
    if not paths['out']:
        paths['out'] = Path(defaults['out_name'])

    print("="*60)
    print(f"Everde Sales Plan Extractor ({region})")
    print("="*60)
    ...
    import subprocess
    base = Path(__file__).parent
    build_script = base / defaults['build']

    if build_script.exists():
        print(f"\nStep 1: Running {build_script.name}...")
        ...
        print(f"WARNING: {defaults['build']} not found — skipping build step")

    wb_candidates = sorted(base.glob(defaults['glob']))
    if not wb_candidates:
        data_root = Path(r"\\192.168.190.10\Claude Sandbox\DataDrops\Sales Plan Review")
        if data_root.exists():
            wb_candidates = sorted(data_root.glob(defaults['glob']))
    if not wb_candidates:
        print(f"ERROR: No output workbook found ({defaults['glob']})")
        sys.exit(1)

    wb_path = wb_candidates[-1]
    print(f"\nStep 2: Extracting from {wb_path.name}...")
    extract_from_existing_workbook(wb_path, paths['out'], region=region)
    print("\nDONE.")
