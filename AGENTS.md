...
**Snapshot 0.5.14 (portal app):** Jonathan freight gap analysis — Top Opps (All/Last Week) filters, Internal Freight tab, Sales Performance + BUD $/mile from `Everde_Freight_Dashboard_2026-05-28.xlsx`; leadership briefing panel on home; optional `ANTHROPIC_ASSISTANT_COMPENDIUM=0` for demo pacing. **0.5.13:** CEO-demo polish — dynamic freight YTD dates from Blob meta. Production: https://everde-operations.vercel.app . See `scripts/freight/CURSOR_Freight_Gap_Analysis.md` and `scripts/freight/FREIGHT_DASHBOARD_DATA.md`.

**Last session (2026-05-21):** Fixed retail exec summary (`action_buckets` + `region_crosstab` in JSON from Region Comparison); republished Blob Wk21. Assistant compendium includes retail + weather JSON. **v0.5.8** on `origin`. Production: https://everde-operations.vercel.app . **Demo:** hard-refresh retail tabs after deploy. **Weather:** daily `Everde-Weather-DailyCheck` (9:30 AM) via `npm run weather:share-pipeline` — share pipeline `fetch` → `build_sales_state` → `build_sales_report` → `build_shared_crosswalk` (Claude ToDo package) + Blob `weather_dashboard_data.json`. XML tasks also in `scripts/windows/claude-scheduler-tasks/`. Manual: `weather:full-pipeline`. **OR Sales Plan:** nav only — workbook/model not on share; see `scripts/sales-plan-review/OR_ROLLOUT.md`. **VM handoff:** `scripts/windows/VM_AGENT_HANDOFF.md`. **DataDrops inventory walk:** TODO.
