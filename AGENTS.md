...
- **Farm inventory (Teams-critical):** The **XXTT** drop in `DataDrops\Sales Inventory Availability\` (`XXTT_INV_QA_LANDSCAPE_INV_PL_*.xls`) is the **primary up-to-date farm / graded on-hand + READY DATE** source for `@Claude` / HD / Lowes via **`get_nursery_supply`**. Prefer it over Inventory Transform or stale Inventory Metrics when answering “what do we have on the farm?”. Agent: **daily 9:00 AM** `Everde-NurserySupply-DailyCheck` (Gmail fetch from Oracle ~3 AM email → drop folder → refresh + Blob + git push). Manual: `npm run nursery:refresh-supply` then `npm run nursery:publish-blob`. Gmail: `XXTT_GMAIL_USER` + `XXTT_GMAIL_APP_PASSWORD` in `.env.local`.
