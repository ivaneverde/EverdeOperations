...
**Snapshot 0.7.3 (portal app):** **Supply Inventory** from `DataDrops\Sales Inventory Availability\` — `nursery:refresh-supply` parses price-list xls into `public/nursery-inventory-dashboard.html` (supply pane). Portal Source path updated. **0.7.2:** Weather / Retail WeeklyDrop; daily scheduler catch-up at 2:30 PM. Production: https://everde-operations.vercel.app .

**Last session (2026-07-09):** Shipped **v0.7.3** — Supply refresh from `XXTT_INV_QA_LANDSCAPE_INV_PL_67618202_1.xls` ($129.9M revenue potential, 4.08M saleable units). After future drops: `npm run nursery:refresh-supply`, commit `public/nursery-inventory-dashboard.html`, push.
