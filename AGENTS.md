...
**Snapshot 0.7.1 (portal app — stable):** **Production & Demand Plan** DEMAND pane repopulated — Inventory Metrics parser reads continuation rows (demand window, ready date, photos), YTD from col 19, first ready-date block only; `renderDemand()` hardened; refreshed from `Inventory Metrics 06 08 26.xlsb`. **Retail:** full store extract (428 stores), All Stores tab + search, assistant/Teams compact. **Teams bot:** Everde Blob snapshot, tool loop, on-demand web. **Freight / sales plan / weather / nursery** assistant compendium. Production: https://everde-operations.vercel.app . Prior: **0.5.15** freight filters + Build Health; **0.5.14** Jonathan gap analysis.

**Last session (2026-05-18):** Shipped **v0.7.1** — fixed blank Demand pane sections (demand window, ready-date pipeline, operational readiness, recommendations). Run `npm run nursery:refresh-demand` after new Inventory Metrics drops. **Demo:** hard-refresh Production & Demand Plan after deploy. **OR Sales Plan:** nav only — see `scripts/sales-plan-review/OR_ROLLOUT.md`. **VM handoff:** `scripts/windows/VM_AGENT_HANDOFF.md`. **DataDrops inventory walk:** TODO.
