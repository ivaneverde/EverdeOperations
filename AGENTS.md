...
**Snapshot 0.5.1 (portal app):** **Production & Demand Plan** refreshes from **`Inventory Metrics\Inventory Metrics *.xlsb`** via `npm run nursery:refresh-demand` (parser in `scripts/nursery/`, embed in `public/nursery-inventory-dashboard.html`). **Freight** and **Sales Plan Review** publish weekly inputs from **`WeeklyDrop`** folders to **Azure Blob** (`freight:extract-publish`, `sales-plan:extract-publish`). **Daily agent** on a VPN PC: Task Scheduler at **8:00 / 9:00 / 1:30** (`scripts/windows/`, handoff `WEEKLY_DROP_AGENT.md`). **Entra auth** for hosted portal; freight HTML Phase C + sales-plan NOR CAL embed unchanged from 0.4.x. See `scripts/freight/FREIGHT_DASHBOARD_DATA.md` and `DASHBOARD_HANDOFF.md`.
