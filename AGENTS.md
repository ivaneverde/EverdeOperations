...
**Snapshot 0.5.6 (portal app):** **Portal compendium assistant** — OpenAI + Claude toggle, freight/sales plan/nursery context with `assistant_facts`. **Sales Plan** dashboard-data falls back to HTML embed when Blob JSON missing. **0.5.2** baseline: Entra auth, weekly agent, nursery refresh, freight/sales plan Blob pipelines. See `scripts/freight/FREIGHT_DASHBOARD_DATA.md` and `scripts/windows/WEEKLY_DROP_AGENT.md`.
