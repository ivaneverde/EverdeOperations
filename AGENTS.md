...
**Snapshot 0.4.3 (portal app):** Same section map as **0.4.1**, plus **freight HTML** resolution: newest `Everde_Freight_Dashboard*.html` by `mtime` in **`public/`** or **`DataDrops/Freight/`**, optional **`FREIGHT_DASHBOARD_HTML`** pin; embed injects CSS to hide legacy **`aside.sidebar`** and YTD **`#sidebar`**, and **`window.activate()`** maps portal tab labels to the static HTML’s **`showTab` ids**. **Root layout** links global CSS so the portal paints without waiting on client JS. Nursery paths unchanged (`Documents`, `public/`, `NURSERY_DASHBOARD_HTML`). See `scripts/freight/FREIGHT_DASHBOARD_DATA.md` and user `DASHBOARD_HANDOFF.md` for data contracts.
