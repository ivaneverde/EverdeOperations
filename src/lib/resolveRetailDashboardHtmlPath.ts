...
const HTML_BASENAME = "Everde_West_Coast_Retail_Opportunity_Dashboard.html";
...
export async function resolveRetailDashboardHtmlPath(): Promise<{
  path: string | null;
  searchedSummary: string;
}> {
  const searched: string[] = [];

  const pinned = process.env.RETAIL_DASHBOARD_HTML?.trim();
  if (pinned) {
    try {
      await fs.access(pinned);
      return { path: pinned, searchedSummary: `RETAIL_DASHBOARD_HTML=${pinned}` };
    } catch {
      searched.push(`RETAIL_DASHBOARD_HTML (missing): ${pinned}`);
    }
  }
  ...
  const dataRoot =
    ...
    DATA_ROOT_UNC;
  const shareDirs = [
    path.join(dataRoot, "SalesOpportunity"),
    path.join(dataRoot, "West Coast Retail Opportunity"),
  ];
  for (const shareDir of shareDirs) {
    searched.push(`${shareDir}\\${HTML_BASENAME}`);
    const fromShare = await globNewestInDir(
      shareDir,
      /^Everde_West_Coast_Retail_Opportunity.*\.html$/i,
    );
    if (fromShare) {
