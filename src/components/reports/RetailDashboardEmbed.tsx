...
export type RetailDashboardEmbedProps = {
  retailHtmlTab?: string | null;
};

export function RetailDashboardEmbed({
  retailHtmlTab = null,
}: RetailDashboardEmbedProps) {
  const iframeRef = useRef<HTMLIFrameElement>(null);
  const tab = (retailHtmlTab ?? "Exec Summary").trim() || "Exec Summary";
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col space-y-3 overflow-hidden">
      ...
      <div className="flex min-h-0 min-w-0 flex-1 overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-sm">
        <iframe
          ref={iframeRef}
          key={iframeKey}
          title="Everde West Coast Retail Opportunity Dashboard"
          className="h-full min-h-0 w-full max-w-full flex-1 border-0"
          src="/api/retail/dashboard-html"
